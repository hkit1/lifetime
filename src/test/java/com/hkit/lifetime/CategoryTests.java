...
public class CategoryTests {
    ...
    void createCategory() throws Exception {
        ...
        mockMvc.perform(post("/api/category/add").param("main", "메인보드").param("sub", "intel").with(csrf())).andExpect(status().isOk());

        assertEquals(1, repository.count());
        assertEquals(2, subRepository.count());

        // 웹에서 스크립트 오류 등으로 인해 상위 카테고리만 추가되거나 하위 카테고리만 추가 될 경우 서버에서 오류를 내는지 확인
        mockMvc.perform(post("/api/category/add").param("main", "메인보드").param("sub", "").with(csrf())).andExpect(status().isBadRequest());
    ...
    void deleteCategory() throws Exception {
        ...
        mockMvc.perform(post("/api/category/delete").param("main", "메인보드").param("sub", "amd").with(csrf())).andExpect(status().isOk());

        assertTrue(repository.findByName("메인보드").isPresent());

        // 하위 카테고리 값이 없을 경우, 하위 카테고리와 함께 상위 카테고리를 삭제하는지 확인
        mockMvc.perform(post("/api/category/delete").param("main", "메인보드").param("sub", "").with(csrf())).andExpect(status().isOk());
    ...
    void updateCategory() throws Exception {
        ...
        mockMvc.perform(post("/api/category/update").param("category", "메인보드").param("oldName", "amd").param("newName", "ami").with(csrf())).andExpect(status().isOk());

        assertTrue(subRepository.findByName("ami").isPresent());

        // 상위 카테고리의 이름이 변경되면서, 하위 카테고리를 유지하는지 확인
        mockMvc.perform(post("/api/category/update").param("category", "").param("oldName", "메인보드").param("newName", "board").with(csrf())).andExpect(status().isOk());
