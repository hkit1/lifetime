...
public class CategoryTests {
    ...
    void create() throws Exception {
        ...
        mockMvc.perform(post("/api/category/add").param("메인보드", "amd"))
    ...
    void delete() throws Exception {
        ...
        mockMvc.perform(post("/api/category/delete").param("메인보드", "amd"))
                .andExpect(status().isOk());

        assertTrue(repository.findByName("메인보드").isPresent());

        // 하위 카테고리 값이 없을 경우, 하위 카테고리와 함께 상위 카테고리를 삭제하는지 확인
        mockMvc.perform(post("/api/category/delete").param("메인보드", ""))
                .andExpect(status().isOk());
