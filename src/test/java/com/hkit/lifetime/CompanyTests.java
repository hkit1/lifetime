...
public class CompanyTests {
  ...
  @Autowired CompanyAccountListRepository listRepository;
  ...
  void createCompany() throws Exception {
    ...
    // 회사 등록 요청이 되는지 확인
    mockMvc
        .perform(post("/api/company/create").params(info).with(csrf()))
  ...
  void acceptCompany() throws Exception {
    ...
    // 회사 등록 요청이 되는지 확인
    mockMvc
        .perform(post("/api/company/create").params(company1).with(csrf()))
        .andExpect(status().isOk());

    mockMvc
        .perform(post("/api/company/create").params(company2).with(csrf()))
        .andExpect(status().isOk());

    // 관리자가 회사 등록 요청 들어온 것을 승인 할 수 있는지 확인
    mockMvc
        .perform(post("/api/company/accept").param("id", company1.getFirst("id")).with(csrf()))
        .andExpect(status().isOk());
    ...
    mockMvc
        .perform(post("/api/company/denied").param("id", company2.getFirst("id")).with(csrf()))
        .andExpect(status().isOk());
  ...
  void connectCompany() throws Exception {
    ...
    assertEquals(
        accountRepository.findAccountById(accountInfo.getFirst("id")).get().getUuid(),
        list.get().getAccount().getUuid());
  }
  ...
  void deleteCompany() throws Exception {
    ...
    mockMvc
        .perform(post("/api/company/delete").param("id", companyInfo.getFirst("id")).with(csrf()))
